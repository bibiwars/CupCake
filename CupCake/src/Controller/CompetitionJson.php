...
class CompetitionJson extends AbstractController
{
    ...
    /**
     * @Route("/competitionupdate", name="cvqdqdre")
     */
    public function updateAction(Request $request,NormalizerInterface $Normalizer)
    {
        $data = json_decode($request->getContent(), true);
        ...
        $em = $this->getDoctrine()->getManager();
        $comp = $em->getRepository(Competition::class)->find($request->get('idCompetition'));
        $comp->setNomCompetition($request->get('nomCompetition'));
